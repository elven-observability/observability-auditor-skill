...
import { renderScoredAlert, renderBatchSummary } from "./lib/pretty.mjs";
...
issues with ❌/⚠️/✅). Pass --json for machine-readable JSON (CI, jq, etc.).
...
async function main() {
  ...
  if (scored.length === 1) {
    process.stdout.write(renderScoredAlert(scored[0]));
  } else {
    process.stdout.write(renderBatchSummary(scored, "alert") + "\n");
    process.stdout.write("Run with --json for the full evidence per rule.\n");
  }
}
